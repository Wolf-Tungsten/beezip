...
def run_sim(sim_path, serializer_path, input_file_path, beezip_mode, beezip_wlog):
    hqt_map = {
        "fast": "1",
        "balanced": "2",
        "better": "4",
    ...
    # run Vbeezip
    args = [sim_path, "+inputFilePath+" + input_file_path, "+hqt+"+hqt_map[beezip_mode], "+enableHashCheck+1", "+wlog+"+str(beezip_wlog)]
...
def simulation_single_file(sim_path, serializer_path, entropy_encoder_path, input_file_path, block_size, output_file_dir, beezip_mode, beezip_wlog):
    ...
    for path in block_path:
        p = Process(target=run_sim, args=(sim_path, serializer_path, path, beezip_mode, beezip_wlog))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--beezip_wlog", type=int, required=True)
    ...
    beezip_wlog = args.beezip_wlog
    
    main_processes = []

    for file_name in os.listdir(input_file_dir):
        input_file_path = os.path.join(input_file_dir, file_name)
        p = Process(target=simulation_single_file, args=(sim_path, serializer_path, entropy_encoder_path, input_file_path, block_size, output_file_dir, beezip_mode, beezip_wlog))
        p.start()
